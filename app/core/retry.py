"""Политика повторов — единственный источник числа повторов и пауз между ними (CLAUDE.md §11).

Перенесена из planers (`app\\core\\retry.py`) как есть. Ей пользуются все сетевые обращения livecraft:
Google Sheets (app\\sheets\\client.py), дальше — YouTube и Google-форма (этап 4). Что именно повторять,
решает вызывающий; здесь — только сколько раз и с какой паузой. Случайная добавка (jitter) разводит повторы
во времени; `rng` — параметром, в тестах фиксированный.
"""
