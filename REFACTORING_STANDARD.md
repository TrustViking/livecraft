...
2. Принимает R1.1a и R1.1b обычным порядком (R1.1 разрезан 25-09-2026, см. раздел 8).
...
- **Задачи порезаны до размера не больше M:** 43 задачи вместо 25 (R1.1 разрезан на R1.1a и R1.1b). Каждая закрывается за один проход Codex.
...
**N18 ✔ Правило «код языка ISO 639-1» в двух местах** (после R1.1a).
...
- Запись личного файла (`VaultStore.save_local`) старый файл не читает — повреждённый файл можно просто заменить. Исправление — в R1.1b: настройщик читает поставочный слой строго, а повреждённый личный — как пустой с пометкой; окно открывается, первое сохранение заменяет файл. `load` и `load_for_setup` идут одним приватным путём; различаются только тем, что делают с повреждённым личным файлом, — иначе замок R2.1b занесёт клон в долг.
- Уточнение D4: текст `DpapiUnavailable` до человека не доходит — его ловят `VaultStore._unwrap_local_key` (лог), `KeysTab._save` (готовый русский текст) и `FormUrlMigration` (имя класса в лог). До человека доходит только `VaultFormatError`; `DpapiUnavailable` получает контракт ошибок в R3.5a.
...
| Правило | Требование Артура | Сейчас ✔ (замок на `89d4afb`) |
|---|---|---|
| E1 Свободные функции | реальный код, а не набор функций | (а) 25, (б) 35, (в) 7 |
| E2 Статические методы | то же | 57 в 39 классах |
| E3 Длина определения | методы короткие | 9 |
| E4 Параметры | объект запроса вместо россыпи | 7 |
| E5 Литералы в телах | ноль хардкода | 996 строк + 20 чисел (замок: 284 функции) |
| E6 Кириллица в коде | весь текст вне кода | 46 |
| E7 Текст исключений | весь текст вне кода | 39 мест в 30 функциях |
| E8 Одно значение — одно объявление | общие константы в одном месте | 37 ключей (31 строка, 6 чисел) / 179 объявлений |
...
| E20 Тесты | тесты тоже эталон | импорты 11, логгеры 20, приватные подмены 7, os/shutil 2, replace 46 (замок: 46 записей) |
...
- **E1 Свободные функции.** Функция уровня модуля — нарушение, если выполняется любой признак:
  ...
  Исключения: точки входа (`app\main.py::run_cli`, `main` пробников — до R4.2b) и функции, которые называет CLAUDE.md. Это `safe_trim_right`, `parse_sheet_datetime` (§0), а для контура B — `mask_stream_key` (§6, инвариант 6) и `parse_iso_start` (§4). `normalize_handle` из §0 после R1.2a — правило `ChannelConfig.key`; в R5.2b его хозяином станет `ChannelHandle`.
...
|---|---|---|---|---|
| R1.1a | Дефекты D1, D2, D3, D5, D6, D7; пример `channels.example.json` — один язык на канал (решение 21) | 3 | — | M |
| R1.1a-2 | Пример `channels.example.json` — один язык на канал и тесты на него (остаток R1.1a: строка `test_main.py` не была в SCOPE LOCK) | R1.1a | — | S |
| R1.1b | Дефекты D4 и D9: причины `VaultFormatError`, повреждённый личный файл ключей чинится в настройщике | 3, D9 | — | M |
...
| R1.2a | Мёртвый код вне merge и нейросети: `core\dates.py`, `core\safe_trim.py`, `core\sheet_text.py`, `sheets\rows.py`, `sources\metadata.py`, `config\loader.py`, фикстура `vault_store` | 4.9 | E1(в), кроме исключений | M |
| R1.2b | Мёртвый код merge и нейросети (профиль повтора `expanded`, `merge_bullet_count_mismatch`, `ECHO_CHECK_PLACEHOLDER`, поля `LlmResponse`, `QualityDiagnostics`, `reject.py` и др.); сверка с донором. `MergePromptRefusal` остаётся рубежом донора до R6.4b | 4.9 | — | M |
| R1.3 | Тесты окна настройщика без случайных `TclError: Can't find a usable init.tcl` при полном прогоне; программа не меняется | R1.2b | — | S |
...
| R2.3 | Золотые (характеризационные) тесты merge и санации: итог и строки лога текущего кода на богатом наборе входов; любое изменение поведения роняет тест с уровнем и id входа. Текущий код равен донору, поэтому для этапа R сверка с донором больше не нужна | R2.3 | — | M |
...
| R5.4 | Сейф: `Vault.overlaid_by`, `only`, `entry` (клон `get` / `origin_of`), `missing_of`; `VaultOrigin`, `FieldSpec`, `Base64Field`, `VaultFile.new`; кольцо `crypto` ↔ `value`; одно перечисление вместо `VaultSource` и `VaultOrigin` — в модуле ниже `crypto`, без кольца | 6.2, N11, N16 | — | S |
...
| R7.1b | `LanguagePicker`, `LanguageDirectory`; `ChannelDraft.languages` — кортеж; тесты языка без Tk; `SetupWindowDriver`; в списке языка помечены варианты вне ISO 639-1 (коды формы и `channels.json`, которые добавляют `LanguageCatalog._unknown` и `including`): после R1.1a загрузчик их отклоняет | 6.7, D5 | — | M |
...
**Порядок.**
1. R1.1a → R1.1b → R2.1a → R2.1b → R1.2a → R1.2b → R1.3 → R2.3 → R2.2a → R2.2b.
...
**Итог:** 45 задач. Боевых прогонов на этапе R не нужно. Исключение — контрольный прогон режима А после R5.1c: он показывает, что пакет и консоль не изменились.
...
- Задача, которая касается app\llm\merges\ или app\texts\, доказывает, что поведение не изменилось: золотые тесты зелёные (.\.venv_livecraft\Scripts\python.exe -m pytest app\tests -m golden -q); до R2.3 — сверкой с донором на тех же входах.
...
- Этап разрезан на 44 задачи объёмом не больше M.
...
8. Сверь числа отчёта с разделом ФАКТЫ: они посчитаны на том же коде. Любое расхождение — найди причину: другое прочтение определения (назови, какое и почему твоё верно) или ошибка сканера (почини сканер). Каждое расхождение — строкой в отчёте.
...
- **Правила.**
  ...
  - E8 — по уточнённому определению раздела 5 (строки — по значению, числа — по имени и значению); E9, E10. Пакет замка `app\tools\code_standard` для E8 и E9 — изолированная область: повторы считаются внутри него и отдельно внутри остального app. Замку по разделу 6 нельзя импортировать общий словарь app, но свои константы внутри пакета он объявляет по одному разу (замечание Codex к R2.1a, решение Коворка).
...
- R2.1a принят (`34442a2`): замок в репозитории, 17 модулей пакета, реестр — 170 записей (E1 63, E2 57, E3 9, E4 7, E12 7, E13 14, E18 12, E19 1, E21 0), исключений 5. Числа и поимённые списки совпали со сверкой Коворка; замок проверен Коворком под Python 3.10 прямо на репозитории — тот же отчёт. Сам пакет замка без долгов: ни свободных функций, ни статических методов, ни литералов в телах, ни `Any`.
- R2.1b принят (`89d4afb`): замок проверяет все 21 правило, реестр — 744 записи. Коворк запустил замок под Python 3.10 прямо на репозитории — тот же отчёт за 3,8 с; пакет замка без долгов (`--files app\tools\code_standard` пуст). Решения Codex: у нарушения есть «места» (модули) — `--files` видит ключи E8, E9, E11, E16; пакет `examples` (данные без кода) — уровень 0 карты; `sealed_modules` для `ui\messages_ru.py`; `SuiteShape` вместо `TestShape` (иначе pytest собирает класс как тест). Числа раздела 5 обновлены по замку.
- R1.2 разрезан на R1.2a и R1.2b (всего 44 задачи): мёртвый код merge требует сверки с донором и сам по себе объёма M. `MergePromptRefusal` не мёртвый код, а рубеж донора со строкой лога `merge_prompt_refused`: `MergePrompt.of` сам отказывает при одном источнике, хотя `MergeJob` до него не доходит. Убирается вместе с перестройкой попытки в R6.4b.
- R1.2a принят (`8dab799`): реестр — 735 записей, E1(в) — только исключения. `normalize_handle` удалена, правило ника — `ChannelConfig.key` до R5.2b (`ChannelHandle`); иначе E1(б) и E12(в) дали бы два новых долга. Промт R2.1a (раздел 11) — выполненный документ и не правится: его строка о `normalize_handle` описывает код на дату задачи.
- Промт R1.2b Коворк сверил с кодом `8dab799` (26-09-2026): все пункты раздела 4.9 `REFACTORING.md` о merge и нейросети подтверждены, кроме уже решённого `MergePromptRefusal`. Добавлены каскадные остатки: заметки обмена OpenAI (`LlmExchange.notes`, `NOTE_*`), `ServiceLineFix.applied`, `BulletNormalization.changed`, `QualityDiagnostics.official_links_heading_mismatch`. `MergeReject.normalized_validation_codes` — не мёртвый код, а пустое преобразование: коды гейта — значения `QualityReasonCode`, ни одно не подходит под шаблон и псевдонимы. Перед удалением Codex проверяет по донору, что производителей `semantic_source_*` и потребителей `normalization_applied` в боевом пути донора нет.
- R1.2b принят (`70c4b1f`): реестр — 728 записей; сверка с донором — 0 расхождений на входах 3.13, 3.14 и 114 075 описаниях эха, логи livecraft до и после побайтно равны. Добавлена задача R1.3 (всего 45): случайные `TclError` при полном прогоне тестов окна — исправление идёт раньше структурных задач. R2.3 поставлена перед R2.2a: входы и скрипты сверок 3.11–3.14 лежат во временной папке Codex, и их надо перенести в репозиторий, пока Windows их не вычистил; R2.3 не зависит от пакета `app\tests\fixtures\`: сверка — инструмент в `app\tools\`, программа тестов не импортирует.
- R1.3 принят (`4fb0f72`): `pytest.ini` с `--capture=sys`, причина доказана опытом, полный прогон зелёный. R2.3 разрезан на R2.3a и R2.3b (всего 46): семь уровней сверки с отдельной донорской стороной не укладываются в M. Ограничения эталона для нового инструмента, которые Коворк проверил по замку: без `get_logger` (E10 — `LogArea` появится в R3.1) и без часов (E17 — `Clock` появится позже); значения, уже объявленные в app, импортируются, а не объявляются заново (E8 не должен вырасти); JSON обмена между процессами и сторона донора — модули-границы E14.
- R2.3 пересмотрен 26-09-2026 по вопросу Артура «зачем такая сложность». Инструмент сверки с донором (R2.3a/R2.3b: второй процесс в окружении restreamer, заплатки донора, исключения замка) решал задачу переноса, а этапу R нужна другая: форма меняется, поведение — нет. Для неё достаточно сравнения с собой: золотые тесты фиксируют итог и строки лога текущего кода, а текущий код уже доказан равным донору (3.11–3.14, R1.2b). Вместо двух задач — одна R2.3 в `app\tests\` (всего 45), без нового кода в программе и без исключений замка. Сверка с донором остаётся только задачам продукта, которые переносят новое поведение (3.14b, 3.15). Архив временных сверок копируется в `tmp\` в той же задаче — для них.
