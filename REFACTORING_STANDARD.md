...
| R2.3a | Сверка с донором одной командой: архив временных сверок, входы всех уровней в репозитории с доказательством равенства, каркас `app\tools\donor_compare\` (сторона донора — отдельный процесс в его окружении), отпечатки строк лога livecraft, чувствительность; уровни `echo` и `publication` | R2.3 | — | M |
| R2.3b | Уровни сверки `answer`, `quality`, `prompt`, `check`, `job` на каркасе R2.3a | R2.3a | — | M |
...
**Порядок.**
1. R1.1a → R1.1b → R2.1a → R2.1b → R1.2a → R1.2b → R1.3 → R2.3a → R2.3b → R2.2a → R2.2b.
...
**Итог:** 46 задач. Боевых прогонов на этапе R не нужно. Исключение — контрольный прогон режима А после R5.1c: он показывает, что пакет и консоль не изменились.
